...
exports.IOSController = exports.AndroidController = void 0;
...
class AndroidController {
    ...
    async swipe(options) {
        ...
        await execAsync(`adb -s ${this.deviceId} shell input swipe ${from.x} ${from.y} ${to.x} ${to.y} ${duration}`);
    }
    /**
     * 텍스트 입력
     */
    ...
    async longPress(x, y, duration = 1000) {
        await execAsync(`adb -s ${this.deviceId} shell input swipe ${x} ${y} ${x} ${y} ${duration}`);
    }
    /**
     * 화면 회전
     */
    ...
    async launchApp(packageName, activityName) {
        if (activityName) {
            await execAsync(`adb -s ${this.deviceId} shell am start -n ${packageName}/${activityName}`);
        }
        else {
     ...
     */
    async findElementAtCoordinates(x, y) {
    ...
    async getCurrentActivity() {
        try {
            const { stdout } = await execAsync(`adb -s ${this.deviceId} shell dumpsys activity activities | grep mResumedActivity`, { timeout: 3000 });
     ...
     */
    async screenshot() {
        const { stdout } = await execAsync(`adb -s ${this.deviceId} exec-out screencap -p | base64`, { maxBuffer: 10 * 1024 * 1024, timeout: 10000 });
        return stdout.trim();
    }
    /**
     * 디바이스 화면 해상도 가져오기
    ...
    async getScreenSize() {
        try {
            const { stdout } = await execAsync(`adb -s ${this.deviceId} shell wm size`, { timeout: 3000 });
...
class IOSController {
     ...
     */
    static async warmUp(appiumServerUrl = 'http://localhost:4723') {
        // 1. Appium 서버 실행 확인
        const { isAppiumServerRunning } = await Promise.resolve().then(() => __importStar(require('../appium/server')));
        const running = await isAppiumServerRunning(appiumServerUrl);
        if (!running) {
            return { success: false, error: 'Appium 서버가 실행되지 않았습니다' };
        }
        // 2. 연결된 iOS 디바이스 탐지
        const { listIOSDevices } = await Promise.resolve().then(() => __importStar(require('../ios/device')));
    ...
    constructor(udid, appiumServerUrl = 'http://localhost:4723', options = {}) {
        this.udid = udid;
        ...
        this.options = {
            ...
            // 좌표 설정
            coordinateOrigin: options.coordinateOrigin ?? process.env.IOS_COORDINATE_ORIGIN ?? 'viewport',
    ...
    async createSession(bundleId) {
        const { createAppiumSession } = await Promise.resolve().then(() => __importStar(require('../appium/server')));
        const { autoConfigureXcode, isXcodeInstalled } = await Promise.resolve().then(() => __importStar(require('../ios/xcode-utils')));
        ...
        // WDA 에러 판별 함수
        const isWDAError = (msg) => msg.includes('xcodebuild failed') ||
        ...
        const isTrustError = (msg) => msg.includes('invalid code signature') ||
            msg.includes('not been explicitly trusted') ||
        ...
        if (!teamId && isRealDevice) {
            throw new Error('WebDriverAgent 세션 생성에 실패했습니다.\n' +
    ...
    async detectTeamId() {
        try {
            ...
            const output = execSync('security find-identity -v -p codesigning 2>/dev/null | head -5', { encoding: 'utf-8', timeout: 5000 });
     ...
     */
    async prepareDevice() {
        const { waitForIOSDevice, isDeviceLockdownReady, waitForDeviceTrust } = await Promise.resolve().then(() => __importStar(require('../ios/device')));
        ...
        if (!usbConnected) {
            throw new Error(`iOS 디바이스를 찾을 수 없습니다: ${this.udid}\n` +
        ...
        if (!lockdownReady) {
            ...
            if (!trusted) {
                throw new Error(`iOS 디바이스 신뢰가 필요합니다: ${this.udid}\n` +
     ...
     */
    async cleanWDADerivedData(fullClean = false) {
    ...
    async closeSession() {
        if (this.sessionId) {
            const { deleteAppiumSession } = await Promise.resolve().then(() => __importStar(require('../appium/server')));
    ...
    async isWDAAlive() {
        if (!this.sessionId)
            return false;
        try {
            const { executeAppiumAction } = await Promise.resolve().then(() => __importStar(require('../appium/server')));
    ...
    async getPageSource() {
        ...
        const { executeAppiumAction } = await Promise.resolve().then(() => __importStar(require('../appium/server')));
        try {
            const result = await executeAppiumAction(this.appiumServerUrl, this.sessionId, 'source', {});
        ...
        try {
            ...
            for (let patternIndex = 0; patternIndex < boundsPatterns.length; patternIndex++) {
                ...
                while ((match = pattern.exec(sourceStr)) !== null) {
                    ...
                    let elemX, elemY, elemWidth, elemHeight;
                    if (patternIndex === 0) {
    ...
    async tapElement(element) {
        ...
        try {
            if (element.accessibilityId) {
                ...
                const elementResponse = await executeAppiumAction(this.appiumServerUrl, this.sessionId, 'element', { using: 'id', value: element.accessibilityId });
                const elementId = elementResponse.value?.ELEMENT || elementResponse.value?.elementId;
                if (elementId) {
                    await executeAppiumAction(this.appiumServerUrl, this.sessionId, `element/${elementId}/click`, {});
                    return;
                }
            }
            if (element.label) {
                ...
                const elementResponse = await executeAppiumAction(this.appiumServerUrl, this.sessionId, 'element', { using: 'xpath', value: xpath });
                const elementId = elementResponse.value?.ELEMENT || elementResponse.value?.elementId;
                if (elementId) {
                    await executeAppiumAction(this.appiumServerUrl, this.sessionId, `element/${elementId}/click`, {});
                    return;
    ...
    async tapAtCoordinates(x, y) {
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            try {
                await executeAppiumAction(this.appiumServerUrl, this.sessionId, 'actions', {
                    actions: [{
                            type: 'pointer',
                            id: 'finger1',
    ...
    async type(text) {
        ...
        const isMultiFieldInput = (fields) => {
            if (fields.length < 2 || fields.length < text.length)
                return false;
            // 같은 y 좌표(±5px) 그룹 찾기
        ...
        const findElementId = async (field) => {
            if (field.accessibilityId) {
                try {
                    const r = await executeAppiumAction(this.appiumServerUrl, this.sessionId, 'element', { using: 'id', value: field.accessibilityId });
            ...
            if (field.label) {
                try {
                    ...
                    const r = await executeAppiumAction(this.appiumServerUrl, this.sessionId, 'element', { using: 'xpath', value: xpath });
                    const eid = r.value?.ELEMENT || r.value?.elementId || null;
                    if (eid)
                        return eid;
                }
        ...
        // ── 헬퍼: 요소의 현재 값 읽기 ──
        const getElementValue = async (elementId) => {
            try {
                const resp = await executeAppiumAction(this.appiumServerUrl, this.sessionId, `element/${elementId}/attribute/value`, {});
        ...
        // ── 헬퍼: 요소 클리어 ──
        const clearElement = async (elementId) => {
            try {
                await executeAppiumAction(this.appiumServerUrl, this.sessionId, `element/${elementId}/clear`, {});
            }
            catch { /* ignore */ }
        };
        // ═════════════════════════════════════════════════════
            ...
            for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
                ...
                if (fields.length > 0) {
                    ...
                    try {
                        ...
                        console.debug('[IOSController] Tap on first field failed, continuing:', e.message?.substring(0, 80));
                    }
                }
                // 한 글자씩 키보드로 입력
                let typedOk = true;
                for (let ci = 0; ci < text.length; ci++) {
                    try {
                        ...
                        console.warn(`[IOSController] [WARN] Multi-field char ${ci} ('${text[ci]}') failed:`, e.message?.substring(0, 80));
                        typedOk = false;
                        break;
                    }
                }
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            ...
            if (elementId) {
                ...
                try {
                    await executeAppiumAction(this.appiumServerUrl, this.sessionId, `element/${elementId}/value`, { text });
                }
                catch (e) {
                    console.warn(`[IOSController] element/value failed:`, e.message?.substring(0, 80));
                }
                // 검증
                await sleep(100);
                ...
                if (actualValue !== null) {
                    ...
                    for (const char of text) {
                        ...
                        if (SINGLE_FIELD_CHAR_DELAY_MS > 0)
                            await sleep(SINGLE_FIELD_CHAR_DELAY_MS);
                    }
                    return;
                }
    ...
    async screenshot() {
        ...
        const { executeAppiumAction } = await Promise.resolve().then(() => __importStar(require('../appium/server')));
        try {
            const result = await executeAppiumAction(this.appiumServerUrl, this.sessionId, 'screenshot', {});
            return result.value || '';
        }
        catch (error) {
    ...
    async back() {
        ...
        await executeAppiumAction(this.appiumServerUrl, this.sessionId, 'back', {});
    }
    /**
    ...
    async home() {
        ...
        try {
            await executeAppiumAction(this.appiumServerUrl, this.sessionId, 'execute_script', {
    ...
    async launchApp(bundleId) {
        ...
        if (!this.sessionId)
            throw new Error('Session not created');
        const { executeAppiumAction } = await Promise.resolve().then(() => __importStar(require('../appium/server')));
        await executeAppiumAction(this.appiumServerUrl, this.sessionId, 'appium/device/activate_app', { bundleId });
    }
    /**
     * 앱 종료
     */
    async stopApp(bundleId) {
        ...
        if (!this.sessionId)
            throw new Error('Session not created');
        const { executeAppiumAction } = await Promise.resolve().then(() => __importStar(require('../appium/server')));
        await executeAppiumAction(this.appiumServerUrl, this.sessionId, 'appium/device/terminate_app', { bundleId });
    }
    /**
     * 앱 캐시/데이터 초기화
     * mobile:clearApp으로 캐시 삭제, 실패 시 removeApp + relaunch fallback
     */
    async clearApp(bundleId) {
        await this.ensureSession();
        if (!this.sessionId)
            throw new Error('Session not created');
        const { executeAppiumAction } = await Promise.resolve().then(() => __importStar(require('../appium/server')));
        ...
        catch { /* 앱이 실행 중이 아닐 수 있음 */ }
        await new Promise(resolve => setTimeout(resolve, 1000));
        try {
            await executeAppiumAction(this.appiumServerUrl, this.sessionId, 'execute_script', { script: 'mobile: clearApp', args: [{ bundleId }] });
            ...
            try {
                await executeAppiumAction(this.appiumServerUrl, this.sessionId, 'execute_script', { script: 'mobile: removeApp', args: [{ bundleId }] });
    ...
    async getWindowSize() {
        ...
        try {
            const result = await executeAppiumAction(this.appiumServerUrl, this.sessionId, 'window/rect', {});
            ...
            try {
                const result = await executeAppiumAction(this.appiumServerUrl, this.sessionId, 'window/size', {});
                if (result.value)
                    return result.value;
            }
            catch { /* both failed */ }
