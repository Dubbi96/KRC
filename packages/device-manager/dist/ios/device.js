...
exports.listIOSDevices = listIOSDevices;
exports.listIOSBundleIds = listIOSBundleIds;
exports.getCurrentIOSBundleId = getCurrentIOSBundleId;
exports.isIOSDeviceConnected = isIOSDeviceConnected;
exports.waitForIOSDevice = waitForIOSDevice;
exports.isDeviceLockdownReady = isDeviceLockdownReady;
exports.waitForDeviceTrust = waitForDeviceTrust;
exports.captureIOSScreen = captureIOSScreen;
 ...
 */
async function listIOSDevicesViaIOReg() {
    try {
 ...
 */
async function listIOSDevicesViaSystemProfiler() {
    try {
 ...
 */
function parseBundleIds(stdout) {
    const bundleIds = [];
    for (const line of stdout.split('\n')) {
        const trimmed = line.trim();
        if (!trimmed || trimmed.startsWith('Total') || trimmed.startsWith('CFBundle') || trimmed.startsWith('-'))
            continue;
 ...
 */
async function listIOSBundleIds(udid) {
 ...
 * idevice 도구 기반
 */
async function getCurrentIOSBundleId(udid) {
    try {
        // SpringBoard의 현재 앱 조회 (idevice 도구 사용)
        const { stdout } = await execAsync(`ideviceinfo -u ${udid} -q com.apple.springboard.cursorcontrol 2>/dev/null || true`, { timeout: 5000 });
 ...
 * iOS 디바이스 연결 확인
 */
async function isIOSDeviceConnected(udid) {
    try {
 ...
 * @returns 연결 성공 여부
 */
async function waitForIOSDevice(udid, timeoutMs = 30000, pollIntervalMs = 2000) {
    ...
    while (Date.now() - start < timeoutMs) {
        ...
        try {
            ...
            if (stdout.includes(udid)) {
                if (lastMethod !== 'idevice_id')
                    console.log(`[iOS] 디바이스 감지됨 (idevice_id): ${udid}`);
                return true;
            }
        }
        catch { /* not found yet */ }
 ...
 */
async function isDeviceLockdownReady(udid) {
    try {
 ...
 */
async function waitForDeviceTrust(udid, timeoutMs = 60000, pollIntervalMs = 3000) {
    const start = Date.now();
    let logged = false;
    while (Date.now() - start < timeoutMs) {
 ...
 * iOS 디바이스 화면 캡처
 */
async function captureIOSScreen(udid, outputPath) {
    await execAsync(`idevicescreenshot -u ${udid} ${outputPath}`);
}
/**
 * iOS 디바이스 로그 수집
 */
async function getIOSLogs(udid, outputPath) {
