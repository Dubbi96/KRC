...
exports.isAppiumServerRunning = isAppiumServerRunning;
exports.startAppiumServer = startAppiumServer;
exports.stopAppiumServer = stopAppiumServer;
exports.createAppiumSession = createAppiumSession;
exports.deleteAppiumSession = deleteAppiumSession;
exports.executeAppiumAction = executeAppiumAction;
const child_process_1 = require("child_process");
/**
 * Appium 서버가 실행 중인지 확인
 */
async function isAppiumServerRunning(serverUrl = 'http://localhost:4723') {
    try {
...
function startAppiumServer(config = {}) {
    ...
    return new Promise(async (resolve, reject) => {
        ...
        // Xcode 경로 자동 설정
        try {
            const { autoConfigureXcode } = await Promise.resolve().then(() => __importStar(require('../ios/xcode-utils')));
        ...
        const appiumProcess = (0, child_process_1.spawn)(appiumCmd, args, {
            stdio: 'pipe',
            cwd: process.cwd(),
            env,
        });
 ...
 * Appium 서버 중지
 */
function stopAppiumServer(process) {
 ...
 * Appium 세션 생성
 */
async function createAppiumSession(serverUrl, capabilities, timeoutMs = 300000) {
    try {
        ...
        if (!response.ok) {
            ...
            if (errorMsg.includes('invalid code signature') || errorMsg.includes('not been explicitly trusted')) {
                throw new Error(`WebDriverAgent 코드 서명이 유효하지 않거나 신뢰되지 않습니다.\n` +
            ...
            if (errorMsg.includes('xcodebuild failed with code 65')) {
                throw new Error(`xcodebuild 실패 (code 65): Xcode 서명 설정 문제입니다.\n` +
            ...
            if (errorMsg.includes('socket hang up') || errorMsg.includes('Could not proxy')) {
                throw new Error(`WDA 프록시 연결 실패 (socket hang up).\n` +
                    `원인 가능성:\n` +
        ...
        if (error.name === 'AbortError' || error.name === 'TimeoutError') {
            throw new Error(`Appium 세션 생성 타임아웃 (${Math.round(timeoutMs / 1000)}초).\n` +
 ...
 * Appium 세션 종료
 */
async function deleteAppiumSession(serverUrl, sessionId) {
 ...
 */
async function executeAppiumAction(serverUrl, sessionId, action, params = {}) {
    // 스크린샷은 GET 요청이어야 함
    ...
    const fetchOptions = {
        ...
        signal: AbortSignal.timeout(action === 'screenshot' ? 5000 :
