 ...
 * Xcode 경로 찾기
 */
export async function findXcodePath(): Promise<string | null> {
  try {
 ...
 * Xcode 정보 가져오기
 */
export async function getXcodeInfo(): Promise<XcodeInfo | null> {
 ...
 * Xcode 경로 설정
 */
export async function setXcodePath(path: string): Promise<boolean> {
  try {
 ...
 */
export function setDeveloperDirEnv(xcodePath: string): void {
 ...
 * Xcode 경로 자동 설정 시도
 */
export async function autoConfigureXcode(): Promise<XcodeInfo | null> {
  const xcodePath = await findXcodePath();
