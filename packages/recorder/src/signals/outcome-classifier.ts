...
export function classifyOutcome(
  ...
  const hasAssertionFailure = failedEvents.some(e =>
    e.assertionResults?.some(ar => !ar.passed),
  );
  ...
  const selectorFailures = failedEvents.filter(e =>
    e.error && retryablePatterns.some(p => e.error!.includes(p)),
  );
  ...
  if (testResult.error) {
    ...
    if (isRetryable) return 'RETRYABLE_FAIL';
  }
