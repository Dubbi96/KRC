...
import { readFileSync, watchFile, unwatchFile, existsSync, writeFileSync } from 'fs';
...
export class NetworkLogCollector {
  ...
  /** 최근 N밀리초 이내의 로그만 반환 */
  getRecentLogs(windowMs: number = DEFAULT_WINDOW_MS): NetworkLogEntry[] {
    const cutoff = Date.now() - windowMs;
    return this.logs.filter(log => log.timestamp >= cutoff);
  }

  /** URL 패턴으로 필터링 (부분 문자열 또는 정규식) */
  findByUrl(pattern: string, isRegex: boolean = false): NetworkLogEntry[] {
    if (isRegex) {
   ...
   * ```
   */
  watchMitmproxyLog(filePath: string, pollIntervalMs: number = 500): void {
    this.stopWatching();
  ...
  snapshot(windowMs?: number): NetworkLogEntry[] {
    if (windowMs) return this.getRecentLogs(windowMs);
    return this.getAll();
  }
