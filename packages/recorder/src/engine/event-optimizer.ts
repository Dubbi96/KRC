...
import type { RecordingEvent, Assertion, WaitForConfig, PreferredLocator } from '../types';
...
export class EventOptimizer {
  ...
  optimize(events: RecordingEvent[]): RecordingEvent[] {
    ...
    let result: RecordingEvent[] = events.map(e => {
      const copy: RecordingEvent = { ...e };
   ...
   */
  private mergeConsecutiveFills(events: RecordingEvent[]): RecordingEvent[] {
   ...
   */
  private insertAutoWaits(events: RecordingEvent[]): RecordingEvent[] {
    const result: RecordingEvent[] = [];
    ...
    for (let i = 0; i < events.length; i++) {
      ...
      if ((current.type === 'click' || current.type === 'fill') &&
        ...
        const waitEvent: RecordingEvent = {
          type: 'wait_for',
   ...
   */
  private stabilizeSelectors(events: RecordingEvent[]): RecordingEvent[] {
    return events.map(event => {
      if (!event.meta?.selectors || event.meta.selectors.length === 0) return event;
   ...
   */
  private enrichPreferredLocators(events: RecordingEvent[]): RecordingEvent[] {
    return events.map(event => {
      // 요소 기반 이벤트만 대상
      if (!['click', 'fill', 'select', 'hover', 'keyboard'].includes(event.type)) return event;

      const elem = event.meta?.element;
      if (!elem) return event;

      const existing = event.meta?.preferredLocators || [];
      const existingKinds = new Set(existing.map(l => l.kind));
      const newLocators: PreferredLocator[] = [...existing];
      ...
      if (newLocators.length === existing.length) return event;
  ...
  private selectorPriority(selector: string): number {
    if (selector.includes('data-testid')) return 0;
    if (selector.startsWith('[role=')) return 1;
    if (selector.includes('aria-label')) return 2;
    if (selector.startsWith('[name=')) return 3;
    if (selector.startsWith('[placeholder=')) return 4;
    if (selector.startsWith('#')) return 5;
    if (selector.includes('[id=')) return 5;
    // class 기반 (tag.class)
    if (/^[a-z]+\.\w/.test(selector)) return 7;
    // nth-of-type 등 구조 의존
    if (selector.includes(':nth')) return 8;
    // 기타
    return 6;
  }
   ...
   */
  private suggestAssertions(events: RecordingEvent[]): RecordingEvent[] {
    // 로그인 플로우 감지를 위한 사전 분석
    const loginSequences = this.detectLoginSequences(events);

    return events.map((event, i) => {
      const suggestions: Array<{ assertion: Assertion; confidence: 'high' | 'medium' | 'low'; reason: string }> = [];
      ...
      if (event.type === 'click') {
        ...
        const isPlayClick =
          /play|재생|▶|시청|watch/i.test(textContent) ||
      ...
      if (event.type === 'click' && i >= 1) {
        ...
        for (let j = i - 1; j >= Math.max(0, i - 5); j--) {
          if (events[j].type === 'fill') { hasPrecedingFills = true; break; }
          if (events[j].type === 'click' || events[j].type === 'navigate') break;
        }
   ...
   */
  private detectLoginSequences(events: RecordingEvent[]): Set<number> {
    const loginNavigateIndices = new Set<number>();

    for (let i = 0; i < events.length; i++) {
      if (events[i].type !== 'fill') continue;
   ...
   */
  private generateDescriptions(events: RecordingEvent[]): RecordingEvent[] {
    return events.map(event => {
      if (event.description) return event;

      const desc = this.buildDescription(event);
      if (!desc) return event;
  ...
  private buildDescription(event: RecordingEvent): string | undefined {
    const elem = event.meta?.element;
    ...
    switch (event.type) {
      ...
      case 'fill': {
        ...
        if (fieldName) return `'${fieldName}' 필드에 입력`;
        return '텍스트 입력';
      }
      ...
      case 'select': {
        ...
        if (fieldName && value) return `'${fieldName}'에서 '${this.truncate(value, 20)}' 선택`;
        return '옵션 선택';
      }

      case 'navigate': {
        if (!event.url) return '페이지 이동';
        try {
          const urlObj = new URL(event.url);
   ...
   */
  private suggestDynamicTracking(events: RecordingEvent[]): RecordingEvent[] {
    // 1단계: 동적 fill 값 수집 — {prefix → varName} 매핑
    const dynamicFillMap = new Map<string, { varName: string; index: number }>();

    for (let i = 0; i < events.length; i++) {
      const event = events[i];
      if (event.type !== 'fill' || !event.value) continue;
      if (!event.value.includes('{{$')) continue;
    ...
    // 2단계: 각 이벤트에 추천 적용
    return events.map((event, i) => {
      const transforms: Array<{ type: string; description: string; field: string; value: string; confidence: 'high' | 'medium' | 'low' }> = [];
   ...
   */
  private extractVarNameFromSelector(selector: string): string {
   ...
   */
  private suggestClickStabilization(events: RecordingEvent[]): RecordingEvent[] {
    return events.map((event, i) => {
      if (event.type !== 'click') return event;

      const fixes: Array<{
        type: 'selector_replace' | 'selector_add' | 'wait_before' | 'wait_after' | 'scroll' | 'force_click' | 'match_text' | 'add_within';
        description: string;
        score: number;
        value: string;
        field?: string;
        withinPayload?: { selector: string; hasText?: string };
      }> = [];
      ...
      if (existingAlts.length <= 1 && textContent && sel) {
        ...
        if (elem?.testId) candidates.push(`[data-testid="${elem.testId}"]`);
        if (elem?.label) candidates.push(`[aria-label="${elem.label}"]`);
        if (elem?.role && textContent) candidates.push(`role=${elem.role}[name="${textContent}"]`);
      ...
      if (!event.within && sel) {
        ...
        if (isGenericSelector) {
          ...
          if (parentScope) {
            ...
            const withinPayload: { selector: string; hasText?: string } = { selector: parentScope };
      ...
      if (topFixes.length > 0) {
        ...
        const bestSelectorFix = topFixes.find(f =>
          (f.type === 'selector_replace' || f.type === 'selector_add') && f.score >= 90
        );
   ...
   */
  private isLikelyDuplicate(selector: string, textContent: string): boolean {
    const lastPart = selector.split(/[\s>]+/).pop()?.trim() || '';

    // ID 셀렉터(#xxx)나 data-testid는 고유하므로 즉시 제외
    if (lastPart.startsWith('#') || /\[data-testid/.test(lastPart)) return false;
   ...
   */
  private findNearestSectionTitle(events: RecordingEvent[], currentIdx: number): string | null {
  ...
  private extractParentFromSelector(selector: string): string | null {
    const parts = selector.split(/\s*>\s*/);
    for (let i = parts.length - 2; i >= 0; i--) {
      const part = parts[i].trim();
      if (/^(section|nav|aside|main|article|header|footer)[\.\[#]?/.test(part)) return part;
      if (/^div\.\w+/.test(part)) return part;
    }
    return null;
  }

  /** CSS selector에서 마지막 요소의 태그명을 추출 */
  private extractLastTag(selector: string): string | null {
  ...
  private getFieldName(event: RecordingEvent): string {
    const elem = event.meta?.element;
    if (!elem) return '';
