...
import { parsePageSource, generateSelector, findElementAtCoordinates, type AndroidUIElement, type AndroidSelector } from './page-source-utils';
 ...
 */
export class AndroidPickServer {
  ...
  private lastPickResult: { selector: AndroidSelector; element: AndroidUIElement } | null = null;

  constructor(
    private controller: any, // AndroidController
  ) {}

  async start(port: number = 8789, dashboardPort?: number, scenarioId?: string): Promise<{ url: string; port: number }> {
  ...
  private async handleApply(req: IncomingMessage, res: ServerResponse): Promise<void> {
    try {
      ...
      const selector = data.selector as AndroidSelector;
