...
import { generateSelector } from './page-source-utils';
 ...
 */
export function suggestAssertionsFromDiff(
  diff: AndroidUITreeDiff,
  tappedElement?: Partial<AndroidUIElement>,
): AssertionSuggestion[] {
  ...
  if (totalChanges > SCREEN_TRANSITION_THRESHOLD) {
    ...
    if (representative) {
      const selector = generateSelector(representative);
      suggestions.push({
        assertion: {
          type: 'android_element_visible' as AssertionType,
  ...
  for (const el of diff.added) {
    if (LAYOUT_ONLY_TYPES.has(el.shortType)) continue;
    ...
    if (hasStableSelector) {
      ...
      suggestions.push({
        assertion: {
          type: 'android_text_contains' as AssertionType,
          target: '',
          expected: el.text,
  ...
  for (const el of diff.removed) {
    ...
    if (hasStableSelector || isTappedElement) {
      suggestions.push({
        assertion: {
          type: 'android_element_not_exists' as AssertionType,
          target: selectorToTarget(selector),
          expected: '',
  ...
  for (const change of diff.changed) {
    ...
    if (LAYOUT_ONLY_TYPES.has(after.shortType)) continue;

    const selector = generateSelector(after);
    const hasTextChange = changes.some(c => c.startsWith('text:'));

    if (hasTextChange && after.text) {
      suggestions.push({
        assertion: {
          type: 'android_element_text_equals' as AssertionType,
          target: selectorToTarget(selector),
          expected: after.text,
...
function selectorToTarget(selector: { strategy: string; value: string }): string {
...
/** 화면 전환 시 대표 요소 선택 */
function findRepresentativeElement(added: AndroidUIElement[]): AndroidUIElement | null {
  const meaningful = added.filter(
    el => !LAYOUT_ONLY_TYPES.has(el.shortType) && (el.text || el.contentDesc || el.resourceId)
  );
...
function matchesElement(el: AndroidUIElement, target: Partial<AndroidUIElement>): boolean {
  if (target.resourceId && el.resourceId === target.resourceId) return true;
  if (target.contentDesc && el.contentDesc === target.contentDesc) return true;
  if (target.text && el.text === target.text && target.shortType && el.shortType === target.shortType) return true;
  return false;
}
