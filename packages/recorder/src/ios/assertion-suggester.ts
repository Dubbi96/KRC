...
export function suggestAssertionsFromDiff(
  diff: UITreeDiff,
  tappedElement?: Partial<IOSUIElement>,
): AssertionSuggestion[] {
  ...
  if (totalChanges > SCREEN_TRANSITION_THRESHOLD) {
    ...
    if (representative) {
      ...
      suggestions.push({
        assertion: {
          type: 'ios_element_visible' as AssertionType,
  ...
  for (const el of diff.added) {
    if (LAYOUT_ONLY_TYPES.has(el.type)) continue;
  ...
  for (const el of diff.removed) {
    ...
    if (hasStableSelector || isTappedElement) {
      suggestions.push({
        assertion: {
          type: 'ios_element_not_exists' as AssertionType,
          target: selectorToTarget(selector),
          expected: '',
  ...
  for (const change of diff.changed) {
    ...
    if (LAYOUT_ONLY_TYPES.has(after.type)) continue;
    ...
    if (hasValueChange && after.value) {
      suggestions.push({
        assertion: {
          type: 'ios_element_value_equals' as AssertionType,
          target: selectorToTarget(selector),
...
function findRepresentativeElement(added: IOSUIElement[]): IOSUIElement | null {
  const meaningful = added.filter(
    el => !LAYOUT_ONLY_TYPES.has(el.type) && el.visible && (el.label || el.name || el.accessibilityId)
  );
...
function matchesElement(el: IOSUIElement, target: Partial<IOSUIElement>): boolean {
  if (target.accessibilityId && el.accessibilityId === target.accessibilityId) return true;
  if (target.name && el.name === target.name) return true;
  if (target.label && el.label === target.label && target.type && el.type === target.type) return true;
  return false;
}
