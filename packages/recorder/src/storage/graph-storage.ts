...
import { normalizeUrlForState } from '../explorer/state-fingerprint';

export class GraphStorage {
  private graphDir: string;

  constructor(baseDir: string = './scenarios') {
    this.graphDir = join(baseDir, '..', 'graphs');
  ...
  getScreenshotDir(graphId: string): string {
    const dir = join(this.graphDir, graphId, 'screenshots');
    if (!existsSync(dir)) mkdirSync(dir, { recursive: true });
    return dir;
  }

  getScreenshotPath(graphId: string, nodeId: string): string {
    return join(this.getScreenshotDir(graphId), `${nodeId}.jpg`);
  }

  // ─── CRUD ──────────────────────────────────────────────

  async save(graph: ExplorationGraph): Promise<void> {
    graph.updatedAt = Date.now();
    const filePath = join(this.graphDir, `${graph.id}.json`);
    await writeFile(filePath, JSON.stringify(graph, null, 2), 'utf-8');
  }

  async load(id: string): Promise<ExplorationGraph | null> {
    const filePath = join(this.graphDir, `${id}.json`);
    try {
  ...
  async list(): Promise<ExplorationGraph[]> {
    if (!existsSync(this.graphDir)) return [];
    const files = (await readdir(this.graphDir)).filter(f => f.endsWith('.json'));
    const results = await Promise.allSettled(
      files.map(async (file) => {
        const data = await readFile(join(this.graphDir, file), 'utf-8');
        return JSON.parse(data) as ExplorationGraph;
      })
    );
    const graphs = results
      .filter((r): r is PromiseFulfilledResult<ExplorationGraph> => r.status === 'fulfilled')
      .map(r => r.value);
    return graphs.sort((a, b) => b.updatedAt - a.updatedAt);
  }

  /** 경량 목록: 노드/엣지 전체 데이터 없이 요약만 반환 */
  async listSummaries(): Promise<Array<{
    id: string; name: string; rootUrl: string; status: string;
  ...
  async createGraph(name: string, rootUrl: string, allowedDomains: string[], config?: ExplorationGraph['config']): Promise<ExplorationGraph> {
    const now = Date.now();
  ...
  addRoot(graph: ExplorationGraph, root: Omit<GraphRoot, 'id' | 'addedAt'>): GraphRoot {
    if (!graph.rootUrls) graph.rootUrls = [];
    const newRoot: GraphRoot = {
      ...root,
      id: randomUUID(),
      addedAt: Date.now(),
    };
  ...
  removeRoot(graph: ExplorationGraph, rootId: string): boolean {
    if (!graph.rootUrls) return false;
  ...
  countNodesForNormalizedUrl(graph: ExplorationGraph, normalizedUrl: string): number {
    return graph.nodes.filter(n => {
      try {
        return normalizeUrlForState(n.url) === normalizedUrl;
      } catch { return false; }
    }).length;
  }
  ...
  normalizeUrl(url: string, ignoreFragments = true): string {
    try {
      const u = new URL(url);
      if (ignoreFragments) u.hash = '';
      // 끝 슬래시 정규화
  ...
  async deleteNode(
  ...
  ): Promise<{ edgesRemoved: number; processesUpdated: string[] }> {
    const nodeIdx = graph.nodes.findIndex(n => n.id === nodeId);
    if (nodeIdx === -1) throw new Error('노드를 찾을 수 없습니다');
    ...
    // 4) 스크린샷 파일 삭제
    if (node.screenshot) {
      const ssPath = join(this.graphDir, node.screenshot);
      try { await unlink(ssPath); } catch { /* 파일 없으면 무시 */ }
    }
