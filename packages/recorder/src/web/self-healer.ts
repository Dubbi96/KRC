...
import { normalizeText, textSimilarity } from './locator-resolver';
...
export class SelfHealer {
  ...
  private async healByRoleNameSimilarity(
    page: Page,
    elem: NonNullable<NonNullable<RecordingEvent['meta']>['element']>,
    opts: ResolverOptions,
    timeout: number,
  ): Promise<SelfHealResult | null> {
    const role = elem.role!;
    const targetName = normalizeText(
      elem.accessibleNameNormalized || elem.label || elem.innerText || elem.textContent,
    );
    if (!targetName) return null;

    try {
      // 같은 role을 가진 모든 visible 요소 수집 (page.evaluate는 브라우저 컨텍스트)
      type RoleCandidateResult = Array<{
        idx: number; text: string; ariaLabel: string | null;
        visible: boolean; enabled: boolean;
        bbox: { x: number; y: number; width: number; height: number } | null;
      }>;
      const candidates: RoleCandidateResult = await page.evaluate(`
      ...
      for (const c of candidates) {
        const candidateName = normalizeText(c.ariaLabel || c.text);
        const sim = textSimilarity(targetName, candidateName);
  ...
  private async healByLabelInput(
    page: Page,
    elem: NonNullable<NonNullable<RecordingEvent['meta']>['element']>,
    opts: ResolverOptions,
    timeout: number,
  ...
  private async healByTagTextSimilarity(
    page: Page,
    elem: NonNullable<NonNullable<RecordingEvent['meta']>['element']>,
    opts: ResolverOptions,
    timeout: number,
    ...
    const tag = elem.type;
    const targetText = normalizeText(elem.textNormalized || elem.textContent || elem.innerText);
    if (!tag || !targetText) return null;

    try {
      type TagCandidateResult = Array<{
        idx: number; text: string; visible: boolean;
        bbox: { x: number; y: number; width: number; height: number } | null;
      }>;
      const candidates: TagCandidateResult = await page.evaluate(`
      ...
      for (const c of candidates) {
        const sim = textSimilarity(targetText, normalizeText(c.text));
  ...
  private async healByBboxProximity(
    page: Page,
    elem: NonNullable<NonNullable<RecordingEvent['meta']>['element']>,
    opts: ResolverOptions,
    timeout: number,
    ...
    try {
      type BboxCandidateResult = Array<{
        idx: number; dist: number; text: string;
        tagName: string; role: string | null;
        bbox: { x: number; y: number; width: number; height: number };
      }>;
      const safeTag = tag.replace(/'/g, "\\'");
      const candidates: BboxCandidateResult = await page.evaluate(`
      ...
      for (const c of candidates) {
        ...
        // 텍스트 유사도 점수: 0~30
        const textScore = targetText ? textSimilarity(targetText, normalizeText(c.text)) * 30 : 0;
        // role 일치 점수: 0 또는 20
