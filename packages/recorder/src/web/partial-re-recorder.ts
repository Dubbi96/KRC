...
import { WebReplayer } from './replayer';
...
import type { RecordingScenario, RecordingEvent, ReplayOptions } from '../types';

export interface PartialReRecordConfig {
  scenarioId: string;
  replaceFromIndex: number;   // 교체 시작 (inclusive)
  replaceToIndex: number;     // 교체 끝 (inclusive)
  scenarioDir: string;
...
export class PartialReRecorder {
  private storage: FileStorage;
  private browser: Browser | null = null;
  private context: BrowserContext | null = null;
  private page: Page | null = null;
  private recorder: WebRecorder | null = null;
  private config: PartialReRecordConfig | null = null;
  private originalScenario: RecordingScenario | null = null;
  private prefixEvents: RecordingEvent[] = [];
  private suffixEvents: RecordingEvent[] = [];

  constructor(storage: FileStorage) {
    this.storage = storage;
  }

  async start(config: PartialReRecordConfig): Promise<void> {
    this.config = config;
    ...
    try {
      ...
      if (!scenario) throw new Error(`Scenario not found: ${config.scenarioId}`);
      if (scenario.platform !== 'web') throw new Error('부분 다시 녹화는 web 시나리오만 지원합니다');
      ...
      if (config.authProfileId) {
        try {
          const authStore = new AuthStore(config.scenarioDir);
      ...
      if (this.prefixEvents.length > 0) {
        ...
        const tempScenario: RecordingScenario = {
          ...scenario,
          id: randomUUID(),
        ...
        const replayer = new WebReplayer();
      ...
      // 5. Recording 모드로 전환
      const recordingScenario: RecordingScenario = {
  ...
  async stop(): Promise<{ scenarioId: string; newEventsCount: number; status: 'done' }> {
    ...
    try {
      ...
      normalizeTimestamps(mergedEvents);
      ...
      if (!scenario) throw new Error('원본 시나리오를 찾을 수 없습니다');
