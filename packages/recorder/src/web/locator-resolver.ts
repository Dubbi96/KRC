...
import { SelfHealer, type SelfHealResult } from './self-healer';
...
export class LocatorResolver {
  ...
  async resolve(
    ...
    const remaining = () => {
      ...
      if (left <= 0) throw new Error('⏱ 스텝 타임아웃');
      return left;
    };

    const withScope = (strategy: string) =>
      scopeDescription ? `${scopeDescription} > ${strategy}` : strategy;
    ...
    const narrowLocator = async (base: Locator): Promise<Locator> => {
      ...
      if (recordedBBox && recordedBBox.width > 0) {
        try {
          ...
          if (count >= 2) return await this.selectNearestByDistance(filtered, recordedBBox, count);
        } catch { /* fallthrough */ }
      }
    ...
    const tryLocator = async (
      locator: Locator, strategyName: string, timeout: number,
      ...
      try {
        ...
        if (Date.now() >= deadline) throw new Error('⏱ 스텝 타임아웃');
        return null;
      }
    };
    ...
    if (healedLocators && healedLocators.length > 0) {
      ...
      for (const healed of sorted) {
        const result = await this.tryPreferredLocator(
          scopeRoot, healed.locator, `healed(${healed.strategy})`, narrowLocator, tryLocator, remaining,
          recordedBBox, elemText,
        );
        if (result) {
    ...
    if (preferred && preferred.length > 0) {
      for (const pref of preferred) {
        const result = await this.tryPreferredLocator(
          scopeRoot, pref, `preferred(${pref.kind})`, narrowLocator, tryLocator, remaining,
          recordedBBox, elemText,
        );
        if (result) {
    ...
    for (const fallbackSel of fallbackSelectors) {
      if (fallbackSel === event.selector) continue;
    ...
    if (elemTextContent && elemTextContent.length <= 80 && event.selector) {
      ...
      if (parentTag) {
        try {
          ...
          const scopedLocator = scopeRoot.locator(parentTag)
            ...
            .first();
          const result = await tryLocator(
            scopedLocator,
            `text-scoped: ${parentTag} >> ${elemType}:has-text("${elemTextContent.substring(0, 30)}")`,
            1500,
          );
          if (result) return result;
        } catch {
      ...
      if (tagName) {
        try {
          ...
          const result = await tryLocator(
            textLocator,
            `tag-text: ${tagName}:has-text("${elemTextContent.substring(0, 30)}")`,
            1500,
          );
          if (result) return result;
        } catch {
  ...
  private async tryPreferredLocator(
    scopeRoot: Page | Locator,
    pref: PreferredLocator,
    strategyLabel: string,
    narrowLocator: (base: Locator) => Promise<Locator>,
    tryLocator: (locator: Locator, name: string, timeout: number) => Promise<ResolveResult | null>,
    remaining: () => number,
    recordedBBox?: { x: number; y: number; width: number; height: number } | null,
    elemText?: string,
  ): Promise<ResolveResult | null> {
    try {
      let baseLocator: Locator | null = null;
      ...
      switch (pref.kind) {
        case 'testid':
          ...
          break;
        case 'role': {
          const roleValue = (pref.role ?? pref.value) as any;
   ...
   */
  private async checkAttributeMismatch(
    locator: Locator,
    elem?: { name?: string; placeholder?: string; [key: string]: any },
  ): Promise<boolean> {
    ...
    try {
      ...
      if (elem.name) {
        ...
        if (actualName !== null && actualName !== elem.name) return true;
      }
      // placeholder 속성 검증
      if (elem.placeholder) {
        const actualPh = await locator.getAttribute('placeholder', { timeout: 500 });
        if (actualPh !== null && actualPh !== elem.placeholder) return true;
      }
    } catch {
  ...
  private async trySemantic(
    scopeRoot: Page | Locator,
    elem: NonNullable<RecordingEvent['meta']>['element'],
    matchText: string | undefined,
    withScope: (s: string) => string,
    remaining: () => number,
    recordedBBox?: { x: number; y: number; width: number; height: number } | null,
  ): Promise<ResolveResult | null> {
    if (!elem) return null;
    ...
    const tryLoc = async (locator: Locator, name: string): Promise<ResolveResult | null> => {
      try {
    ...
    if (elem.role) {
      ...
      const loc = roleName
        ? scopeRoot.getByRole(elem.role as any, { name: roleName, exact: false })
        : scopeRoot.getByRole(elem.role as any);
  ...
  private async selectNearestByDistance(
    locator: Locator,
    recordedBox: { x: number; y: number; width: number; height: number },
    count: number,
  ): Promise<Locator> {
  ...
  private buildDetailedError(
    event: RecordingEvent,
    elem: NonNullable<NonNullable<RecordingEvent['meta']>['element']> | undefined,
    scopeDescription: string,
    matchText: string | undefined,
    fallbackSelectors: string[],
  ): Error {
    const triedMethods: string[] = [];
    if (scopeDescription) triedMethods.push(`스코프: ${scopeDescription}`);

    const preferred = event.meta?.preferredLocators;
    if (preferred && preferred.length > 0) {
      triedMethods.push(`preferredLocators: ${preferred.map(p => `${p.kind}="${p.value?.substring(0, 30)}"`).join(', ')}`);
    }
    if (event.selector) triedMethods.push(`CSS 셀렉터: "${event.selector}"`);
    if (fallbackSelectors.length > 0) {
      triedMethods.push(`CSS 폴백: ${fallbackSelectors.map(s => `"${s}"`).join(', ')}`);
    }
    if (elem) {
      const semanticTried: string[] = [];
      if (elem.testId) semanticTried.push(`testId="${elem.testId}"`);
      if (elem.role) semanticTried.push(`role="${elem.role}"${elem.label ? ` name="${elem.label}"` : ''}`);
      if (elem.label) semanticTried.push(`label="${elem.label}"`);
      if (elem.placeholder) semanticTried.push(`placeholder="${elem.placeholder}"`);
      if (elem.title) semanticTried.push(`title="${elem.title}"`);
      const errorText = matchText || elem.innerText || elem.textContent;
      if (errorText) semanticTried.push(`text="${errorText.substring(0, 50)}"`);
      if (semanticTried.length) triedMethods.push(`시맨틱: ${semanticTried.join(', ')}`);
    }
...
function extractParentSection(selector: string): string | null {
