import { chromium, firefox, webkit, type Browser, type BrowserContext, type Page, type Locator, type Dialog } from 'playwright';
import type { RecordingScenario, RecordingEvent, ReplayOptions, EventResult, TestResult, NetworkLogEntry, HealedLocator } from '../types';
...
import {
  ...
  executeExtractData, executeWaitFor, evaluateIfCondition,
  executeImageMatch, executeOcrExtract, executeCheckEmail, type ExecutionContext
} from '../engine/step-executors';
import { resolveDeviceConfig, toContextOptions } from './device-presets';
import { AuthStore } from '../dashboard/auth-store';
import { PageRegistry } from './page-registry';
import { LocatorResolver, type ResolveResult, type ResolverOptions } from './locator-resolver';
...
export class WebReplayer {
  ...
  private locatorResolver = new LocatorResolver();

  /** healedLocator 결과를 시나리오 이벤트에 반영 (다음 실행에서 우선 사용) */
  private applyHealedLocator(event: RecordingEvent, healed: HealedLocator): void {
    if (!event.meta) event.meta = {};
    if (!event.meta.healedLocators) event.meta.healedLocators = [];

    // 동일 전략의 기존 기록이 있으면 successCount 업데이트
    const existing = event.meta.healedLocators.find(
      h => h.locator.kind === healed.locator.kind && h.locator.value === healed.locator.value,
    );
    if (existing) {
  ...
  private bumpHealedLocatorSuccess(event: RecordingEvent, usedHealed: HealedLocator): void {
    const healedList = event.meta?.healedLocators;
    if (!healedList) return;
    const match = healedList.find(
      h => h.locator.kind === usedHealed.locator.kind && h.locator.value === usedHealed.locator.value,
    );
    if (match) {
      match.successCount++;
  ...
  private buildResolverOptions(
    page: Page, event: RecordingEvent, vars: VariableContext, deadline: number,
  ): ResolverOptions {
  ...
  async replay(scenario: RecordingScenario, options: ReplayOptions = {}): Promise<TestResult> {
    const {
      speed = 1.0, delayBetweenEvents = 100,
      takeScreenshots = false, reportDir = './reports',
      stopOnFailure = true,
    } = options;
    ...
    if (options.testDataSetName && scenario.testData) {
      ...
      if (dataSet) variables.merge(dataSet.variables);
    }
    ...
    const deviceConfig = await resolveDeviceConfig(effectiveDeviceType);
    const contextOptions = effectiveDeviceType
      ? toContextOptions(deviceConfig)
    ...
    if (options.authProfileId && !options.existingContext) {
      try {
        const authStore = new AuthStore(reportDir.replace(/[/\\]reports$/, '/scenarios'));
    ...
    try {
      ...
      const tsFixed = normalizeTimestamps(events, { maxGap: 10000, defaultGap: 300 });
      if (tsFixed > 0) console.log(`[WebReplayer] timestamp 정규화: ${tsFixed}개 스텝 보정됨`);
      ...
      for (let i = 0; i < events.length; i++) {
        ...
        if (event.type === 'for_each_start') {
          ...
          for (let iter = 0; iter < maxIter; iter++) {
            ...
            for (const innerEvent of innerSteps) {
              if (innerEvent.disabled) continue;
              ...
              if (result.status === 'failed' && stopOnFailure) { loopBroken = true; break; }
            }
            if (loopBroken) break;
          }

          if (loopBroken && stopOnFailure) break;
        ...
        if (event.type === 'popup_opened') {
          ...
          if (!popupPage) {
            const registeredPages = new Set(
              [...this.pageRegistry.getAll().values()],
            );
            const unregistered = context.pages().find((p: Page) => !registeredPages.has(p));
            if (unregistered) {
              if (expectedPageId) {
        ...
        if (nextEvent?.type === 'dialog' && nextEvent.dialogConfig && !nextEvent.disabled) {
          ...
          if (triggerResult.status === 'failed' && stopOnFailure) break;
          continue;
        }
   ...
   */
  private resolvePageForEvent(event: RecordingEvent, fallbackPage: Page): Page {
    const pageId = event.meta?.pageId;
    if (!pageId || pageId === 'main') return fallbackPage;
    ...
    for (let j = allPages.length - 1; j >= 0; j--) {
      if (!allPages[j].isClosed()) return allPages[j];
    }
  ...
  private static STEP_TIMEOUT = 30_000;

  /** 이벤트 특성에 따른 동적 타임아웃 계산 */
  private static getStepTimeout(event: RecordingEvent): number {
    // wait_for(url_change)에서 외부 도메인이면 60초
    if (event.type === 'wait_for' && event.waitForConfig?.waitType === 'url_change') {
      if (event.meta?.isExternalDomain) return 60_000;
  ...
  private async replayEvent(
    page: Page, event: RecordingEvent, index: number,
    screenshotDir: string | undefined, execCtx: ExecutionContext,
    globalScreenshots: boolean,
  ): Promise<EventResult> {
  ...
  private async executeStep(
    page: Page, event: RecordingEvent, index: number,
    screenshotDir: string | undefined, execCtx: ExecutionContext,
    globalScreenshots: boolean, start: number, deadline: number,
  ): Promise<EventResult> {
    ...
    try {
      ...
      const checkDeadline = () => {
        if (Date.now() >= deadline) throw new Error(`⏱ 스텝 타임아웃 (${WebReplayer.STEP_TIMEOUT / 1000}초 초과)`);
      };
      ...
      switch (event.type) {
        ...
        case 'click': {
          ...
          try {
            ...
            if (!extraResult.resolvedBy) extraResult.resolvedBy = clickResult.resolvedBy;
        ...
        case 'fill': {
          if (event.value !== undefined) {
            ...
            if (fillResult.healedLocator) this.applyHealedLocator(event, fillResult.healedLocator);
            if (fillResult.usedHealedLocator) this.bumpHealedLocatorSuccess(event, fillResult.usedHealedLocator);
            checkDeadline();
            ...
            const isCheckable = elemRole === 'checkbox' || elemRole === 'radio' ||
              (elemType === 'input' && await fillResult.locator.getAttribute('type').catch(() => '')
                .then((t: string | null) => t === 'checkbox' || t === 'radio'));
            if (isCheckable) {
        ...
        case 'select': {
          if (event.value !== undefined) {
            ...
            if (selectResult.healedLocator) this.applyHealedLocator(event, selectResult.healedLocator);
            if (selectResult.usedHealedLocator) this.bumpHealedLocatorSuccess(event, selectResult.usedHealedLocator);
            checkDeadline();
        ...
        case 'api_request':
          ...
          break;
        case 'set_variable':
          extraResult = await executeSetVariable(event, execCtx);
          break;
        case 'run_script':
          extraResult = await executeRunScript(event, execCtx);
          break;
        case 'assert': {
        ...
        // RPA 확장
        case 'extract_data':
          extraResult = await executeExtractData(event, execCtx);
          break;
        case 'keyboard': {
          ...
          if (kbConfig) {
            if (kbConfig.selector) {
              ...
              if (kbResult.healedLocator) this.applyHealedLocator(event, kbResult.healedLocator);
              if (kbResult.usedHealedLocator) this.bumpHealedLocatorSuccess(event, kbResult.usedHealedLocator);
        ...
        case 'hover': {
          ...
          if (hoverResult.healedLocator) this.applyHealedLocator(event, hoverResult.healedLocator);
          if (hoverResult.usedHealedLocator) this.bumpHealedLocatorSuccess(event, hoverResult.usedHealedLocator);
          checkDeadline();
        ...
        case 'ocr_extract': {
          ...
          const ocrReportDir = screenshotDir ? join(screenshotDir, '..') : undefined;
          extraResult = await executeOcrExtract(event, execCtx, ocrReportDir);
          break;
        }
      ...
      if (screenshotDir) {
        if (hasError) {
          ...
          screenshot = join(screenshotDir, `step_${String(index + 1).padStart(3, '0')}.png`);
          await page.screenshot({ path: screenshot }).catch(() => {});
        }
      }
      ...
      return {
        ...
        stepNo: event.stepNo, description: event.description,
        resolvedBy: extraResult.resolvedBy as string | undefined,
        error: errorMsg,
      ...
      if (isTimeout) {
        ...
        if (event.selector || event.meta?.selectors) {
          const tried: string[] = [];
          if (event.selector) tried.push(`primary: "${event.selector}"`);
          const fallbacks = event.meta?.selectors || [];
          if (fallbacks.length > 0) tried.push(`fallbacks: ${fallbacks.length}개`);
          const elem = event.meta?.element;
          if (elem) {
            const semInfo: string[] = [];
            if (elem.testId) semInfo.push(`testId="${elem.testId}"`);
            if (elem.role) semInfo.push(`role="${elem.role}"`);
            if (elem.label) semInfo.push(`label="${elem.label}"`);
            if (elem.placeholder) semInfo.push(`placeholder="${elem.placeholder}"`);
            if (semInfo.length) tried.push(`semantic: ${semInfo.join(', ')}`);
          }
  ...
  private safeSelector(sel: string): string {
    ...
    if (attrMatch) {
      ...
      if (escaped !== attrMatch[2]) return `[${attrMatch[1]}="${escaped}"]`;
    }
    return sel;
  }

  private escapeAttributeValue(val: string): string {
   ...
   */
  private async restoreScrollPosition(page: Page, event: RecordingEvent): Promise<void> {
    const ctx = event.meta?.pageContext;
    if (!ctx || ctx.scrollY === undefined) return;
   ...
   */
  private async resolveWithScrollDiscovery(
    page: Page,
    ...
    try {
      ...
      if (opts.deadline - Date.now() < 2000) throw firstError;

      // 2차: 점진적 스크롤 탐색
      const dims: { scrollHeight: number; viewportHeight: number } = await page.evaluate(
      ...
      for (const delta of [0, -step, step, -step * 2, step * 2]) {
        ...
        if (!positions.includes(y)) positions.push(y);
      }
      ...
      for (const y of positions) {
        if (opts.deadline - Date.now() < 1500) break; // 시간 부족하면 중단
  ...
  private findMatchingEnd(
    events: RecordingEvent[], startIdx: number,
