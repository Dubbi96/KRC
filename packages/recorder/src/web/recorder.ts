...
import type { RecordingScenario, RecordingEvent, RecordingConfig, DialogConfig, DialogAction } from '../types';
 ...
 */
export const SPA_NAVIGATION_SCRIPT = `
(function() {
...
export class WebRecorder {
   ...
   */
  private async setupPageListeners(page: Page, pageId: string): Promise<void> {
    // exposeFunction은 page 단위 — 팝업에도 개별 등록 필요
    await page.exposeFunction('__katabRecordEvent', (eventData: string) => {
      try {
        const event = JSON.parse(eventData) as RecordingEvent;
    ...
    page.on('dialog', async (dialog: Dialog) => {
      ...
      const dialogType = dialog.type() as DialogConfig['dialogType'];
      ...
      const action: DialogAction = 'accept';
   ...
   */
  private setupPopupListener(): void {
    if (!this.context) return;

    this.context.on('page', async (popup: Page) => {
      ...
      try { await popup.waitForLoadState('domcontentloaded', { timeout: 10000 }); } catch { /* non-critical */ }
  ...
  async attachToPage(
    ...
    await context.addInitScript({ content: RECORDING_SCRIPT });
    await context.addInitScript({ content: SPA_NAVIGATION_SCRIPT });
    this.setupPopupListener();

    // 현재 페이지에도 스크립트 직접 주입 (addInitScript는 다음 네비게이션부터 적용)
    await page.evaluate(RECORDING_SCRIPT).catch(() => {});
    await page.evaluate(SPA_NAVIGATION_SCRIPT).catch(() => {});
  }
  ...
  async start(): Promise<string> {
    ...
    const deviceConfig = await resolveDeviceConfig(this.config.deviceType);
    ...
    if (this.config.authProfileId) {
      const authStore = new AuthStore(this.config.outputDir || './scenarios');
   ...
   */
  private recordEvent(event: RecordingEvent, forceRecord = false): void {
    if (!this.scenario) return;
    if (this.isPaused && !forceRecord) return;

    // 모든 이벤트에 pageId 보장 (마커 이벤트 등 수동 생성 이벤트 포함)
    if (!event.meta) event.meta = {};
    if (!event.meta.pageId) event.meta.pageId = 'main';
   ...
   */
  async resume(scenarioId: string): Promise<string> {
    if (this.scenario) throw new Error('Recording already started');
    ...
    const deviceConfig = await resolveDeviceConfig(deviceType);
