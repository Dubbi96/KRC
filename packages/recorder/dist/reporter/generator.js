...
class ReportGenerator {
    generateHTML(result, outputDir) {
        const reportPath = (0, path_1.join)(outputDir, 'report.html');
        if (!(0, fs_1.existsSync)(outputDir))
            (0, fs_1.mkdirSync)(outputDir, { recursive: true });
        (0, fs_1.writeFileSync)(reportPath, this.buildHTML(result), 'utf-8');
        return reportPath;
    }
    generateJSON(result, outputDir) {
        const reportPath = (0, path_1.join)(outputDir, 'result.json');
        if (!(0, fs_1.existsSync)(outputDir))
            (0, fs_1.mkdirSync)(outputDir, { recursive: true });
        (0, fs_1.writeFileSync)(reportPath, JSON.stringify(result, null, 2), 'utf-8');
        return reportPath;
    }
    buildHTML(result) {
  ...
  Generated by Katab &mdash; Recording-based QA Test Platform
    ...
    buildEventRow(ev, i) {
        ...
        const apiHtml = ev.apiResponse
          ...
          <pre style="margin:6px 0 0;max-height:200px;overflow:auto">${this.esc(typeof ev.apiResponse.body === 'string'
                ? ev.apiResponse.body.substring(0, 1000)
                : JSON.stringify(ev.apiResponse.body, null, 2).substring(0, 1000))}</pre>
        </div>`
            : '';
        // 선택자 해결 전략
    ...
    /** 최종 변수 상태 섹션 */
    buildVariablesSection(variables) {
        const entries = Object.entries(variables);
        if (entries.length === 0)
            return '';
        const rows = entries.map(([k, v]) => `<tr><td><code>${this.esc(k)}</code></td><td>${this.esc(String(v).substring(0, 200))}</td></tr>`).join('');
        return `
      <div class="var-table">
