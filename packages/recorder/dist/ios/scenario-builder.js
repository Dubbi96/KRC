...
exports.IOSScenarioBuilder = void 0;
exports.iosScenario = iosScenario;
exports.navigationAssertionSet = navigationAssertionSet;
exports.networkAssertionSet = networkAssertionSet;
exports.listResultAssertionSet = listResultAssertionSet;
const crypto_1 = require("crypto");
// ─── Selector → iosSelector 변환 ─────────────────────────
function toIOSSelector(def) {
    if (def.accessibilityId)
        return { strategy: 'accessibility_id', value: def.accessibilityId };
    if (def.name)
        return { strategy: 'name', value: def.name };
    if (def.label)
        return { strategy: 'label', value: def.label };
    if (def.xpath)
        return { strategy: 'xpath', value: def.xpath };
    throw new Error('IOSSelectorDef에 accessibilityId, name, label, xpath 중 하나를 지정해야 합니다.');
}
// ─── Builder ────────────────────────────────────────────
class IOSScenarioBuilder {
    events = [];
    config;
    name;
    stepCounter = 0;
     ...
     */
    tap(selector, description, fallbackCoordinates) {
        this.events.push({
            type: 'tap',
            timestamp: Date.now(),
     ...
     * 스와이프
     */
    swipe(from, to, description, duration) {
        this.events.push({
            type: 'swipe',
            timestamp: Date.now(),
            stepNo: ++this.stepCounter,
     ...
     * 요소 가시성 확인
     */
    assertVisible(selector, message) {
     ...
     * 요소 부재 확인
     */
    assertNotExists(selector, message) {
        this.addAssertionStep({
     ...
     * 텍스트 존재 확인
     */
    assertTextPresent(text, message) {
        this.addAssertionStep({
     ...
     */
    assertNoErrorText(additionalTexts, message) {
     ...
     * 리스트 아이템 존재 확인 (최소 N개)
     */
    assertListCount(minCount = 1, elementType = 'Cell', message) {
        this.addAssertionStep({
            type: 'ios_list_count',
            target: '',
     ...
     * 요소 대기 (wait_for)
     */
    waitForVisible(selector, timeout = 30000, description) {
        this.events.push({
            type: 'wait_for',
            timestamp: Date.now(),
            stepNo: ++this.stepCounter,
     ...
     * 텍스트 대기 (wait_for)
     */
    waitForText(text, timeout = 30000, description) {
        this.events.push({
            type: 'wait_for',
            timestamp: Date.now(),
            stepNo: ++this.stepCounter,
     ...
     * 요소 사라짐 대기 (로딩 스피너 등)
     */
    waitForGone(selector, timeout = 30000, description) {
        this.events.push({
            type: 'wait_for',
            timestamp: Date.now(),
            stepNo: ++this.stepCounter,
     ...
     * @param options - 필터 옵션
     */
    assertRequestSent(urlPattern, options, message) {
        this.addAssertionStep({
     ...
     * API 응답 상태 코드 확인
     */
    assertResponseStatus(urlPattern, expectedStatus, options, message) {
        this.addAssertionStep({
     ...
     */
    assertJsonPath(urlPattern, jsonPath, op, value, options, message) {
        this.addAssertionStep({
     ...
     */
    assertImageLoads(apiUrlPattern, imageUrlJsonPath, sampleCount = 3, message) {
        this.addAssertionStep({
     ...
     */
    assertNoNetworkErrors(urlPattern, options, message) {
        this.addAssertionStep({
...
exports.IOSScenarioBuilder = IOSScenarioBuilder;
// ─── Factory ────────────────────────────────────────────
/**
 ...
 * ```
 */
function iosScenario(name, config) {
 ...
 */
function navigationAssertionSet(screenTitle, errorTexts) {
    return [
        {
            type: 'ios_text_contains',
            target: '',
 ...
 */
function networkAssertionSet(apiUrlPattern, expectedStatus = 200, jsonChecks) {
    const assertions = [
        {
            type: 'network_request_sent',
            target: '',
 ...
 * 리스트/검색 결과 검증 세트
 */
function listResultAssertionSet(minCount = 1, emptyStateText) {
    const assertions = [
        {
            type: 'ios_list_count',
            target: '',
