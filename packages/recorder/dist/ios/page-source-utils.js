...
exports.parsePageSource = parsePageSource;
exports.generateSelector = generateSelector;
exports.diffUITrees = diffUITrees;
...
function elementKey(el) {
    ...
    if (el.accessibilityId)
        return `aid:${el.accessibilityId}`;
    // type + name 조합
    if (el.name)
        return `${el.type}:name:${el.name}`;
    // type + label 조합
    if (el.label)
        return `${el.type}:label:${el.label}`;
 ...
 */
function diffUITrees(before, after) {
...
function searchElements(elements, query) {
    ...
    return elements.filter(el => {
        if (el.label && el.label.toLowerCase().includes(q))
            return true;
        if (el.name && el.name.toLowerCase().includes(q))
            return true;
        if (el.value && el.value.toLowerCase().includes(q))
            return true;
        if (el.accessibilityId && el.accessibilityId.toLowerCase().includes(q))
            return true;
        return false;
    });
}
 ...
 */
function findElementBySelector(elements, selector) {
    switch (selector.strategy) {
        ...
        case 'xpath': {
            ...
            return candidates.find(el => el[attr] === val) || null;
        }
        default:
            return null;
    }
}
//# sourceMappingURL=page-source-utils.js.map