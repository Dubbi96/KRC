...
const network_collector_1 = require("../engine/network-collector");
...
class IOSReplayer {
    ...
    async replay(scenario, options = {}) {
        ...
        // ─── 네트워크 로그 수집기 초기화 ───
        this.networkCollector = new network_collector_1.NetworkLogCollector();
        ...
        try {
            ...
            execCtx.appiumKeepAlive = async () => {
                try {
                    await controller.getPageSource();
                }
                catch { /* ignore */ }
            };
            ...
            if (tsFixed > 0)
                console.log(`[IOSReplayer] timestamp 정규화: ${tsFixed}개 스텝 보정됨`);
            ...
            for (let i = 0; i < events.length; i++) {
                ...
                if (event.type === 'if_start') {
                    ...
                    try {
                        lastPageSource = await controller.getPageSource() || undefined;
                    }
                    catch { /* 무시 */ }
                    ...
                    if (wait > 0) {
                        ...
                        if (wait > 10000) {
                            ...
                            for (let c = 0; c < chunks; c++) {
                                ...
                                if (c < chunks - 1) {
                                    try {
                                        await controller.getWindowSize();
                                    }
                                    catch { /* keep-alive ping 실패 무시 */ }
                                }
                            }
                        }
                        else {
                ...
                if (process.env.RETURN_TO_HOME !== 'false') {
                    try {
                        ...
                        if (this.currentBundleId && scenario.existingAppiumSessionId) {
                            const { executeAppiumAction } = await Promise.resolve().then(() => __importStar(require('@katab/device-manager')));
                            await executeAppiumAction(scenario.appiumServerUrl || 'http://localhost:4723', scenario.existingAppiumSessionId, 'appium/device/terminate_app', { bundleId: this.currentBundleId });
            ...
            switch (event.type) {
                ...
                case 'tap': {
                    ...
                    if (event.iosSelector) {
                        ...
                        if (tapped) {
                            ...
                            throw new Error(`Cannot find element by selector (${event.iosSelector.strategy}=${event.iosSelector.value}) and no coordinates available.`);
                        }
                    }
                        ...
                        const isAmbiguous = isKeyboardContainer || (el.type === 'Other' && !el.accessibilityId && ((el.name && el.name === el.label) || // name과 label이 같으면 앱 자체일 가능성 높음
                            (!el.name && !el.label)));
                        ...
                        throw new Error('Cannot replay tap: no iosSelector, element metadata, or coordinates. ' +
                ...
                case 'scroll':
                    if (event.from && event.to) {
                        ...
                        throw new Error('Cannot replay swipe/scroll: missing from/to coordinates. ' +
                            'Re-record this scenario using mirror mode (--mirror).');
                    }
                    break;
                case 'type':
                ...
                case 'clear_app': {
                    ...
                    if (!bundleId)
                        throw new Error('Cannot clear app: no bundleId specified');
    ...
    async tapBySelector(controller, selector, options = {}) {
        ...
        for (const strat of strategies) {
            for (let attempt = 0; attempt <= maxRetries; attempt++) {
                try {
                    const elementResponse = await executeAppiumAction(serverUrl, sessionId, 'element', { using: strat.using, value: strat.value });
                    ...
                    try {
                        const rectResp = await executeAppiumAction(serverUrl, sessionId, `element/${elementId}/rect`, {});
                    ...
                    await executeAppiumAction(serverUrl, sessionId, `element/${elementId}/click`, {});
                    return true;
                }
                catch {
                    if (attempt < maxRetries) {
     ...
     */
    async waitForUISettle(controller, prevPageSource, minWait = 500, maxWait = 3000) {
        // 최소 대기
...
exports.IOSReplayer = IOSReplayer;
