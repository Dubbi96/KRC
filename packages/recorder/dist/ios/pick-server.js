...
class IOSPickServer {
    ...
    async start(port = 8788, dashboardPort, scenarioId, mode, stepIdx, batchPlanId) {
        ...
        if (this.batchPlanId && this.dashboardPort) {
            try {
                ...
                if (resp.ok) {
                    this.batchPlan = await resp.json();
     ...
     * body: { x1, y1, x2, y2 }
     */
    async handlePickRegion(req, res) {
        try {
            const body = await this.readBody(req);
    ...
    async handleSearch(req, res) {
        try {
            ...
            const elements = (0, page_source_utils_1.parsePageSource)(xml);
            const results = (0, page_source_utils_1.searchElements)(elements, query);
     ...
     * 좌표만 대시보드에 전송 (셀렉터 없이)
     */
    async handleApplyCoordinates(req, res) {
        try {
            const body = await this.readBody(req);
            const { x, y } = JSON.parse(body);
     ...
     */
    async handleBatchApply(req, res) {
        try {
            const body = await this.readBody(req);
...
exports.IOSPickServer = IOSPickServer;
