...
exports.findElementAtCoordinates = findElementAtCoordinates;
...
function elementKey(el) {
    ...
    if (el.resourceId)
        return `rid:${el.resourceId}`;
    // type + content-desc 조합
    if (el.contentDesc)
        return `${el.shortType}:desc:${el.contentDesc}`;
    // type + text 조합
    if (el.text)
        return `${el.shortType}:text:${el.text}`;
 ...
 */
function findElementAtCoordinates(elements, x, y) {
...
function findElementBySelector(elements, selector) {
    switch (selector.strategy) {
        ...
        case 'xpath': {
            ...
            return candidates.find(el => el[key] === val) || null;
        }
        default:
            return null;
    }
