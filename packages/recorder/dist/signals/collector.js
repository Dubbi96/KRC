 ...
 */
function collectSignals(testResult, platform) {
    ...
    const fallbackRecords = records.filter(r => r.usedCoordinateFallback || r.usedForceClick || isFallbackResolution(r.resolvedBy, resolvedPlatform));
 ...
 */
function analyzeEventResult(event, platform) {
    if (!event.resolvedBy)
        return null;
 ...
 */
function isCoordinateFallback(resolvedBy) {
 ...
 * force click 사용 여부
 */
function isForceClick(resolvedBy) {
 ...
 */
function isFallbackResolution(resolvedBy, platform) {
    if (platform === 'web') {
        // 1순위: primary, css → 확정 non-fallback
        if (resolvedBy.startsWith('primary:'))
            return false;
        if (resolvedBy.startsWith('css:'))
            return false;
        // preferred(...) 전략: Playwright getByRole/getByText/getByTestId 등 → 신뢰도 높음
        if (resolvedBy.startsWith('preferred('))
            return false;
        // semantic 중 구조 기반 → 신뢰도 높음
        if (resolvedBy.startsWith('semantic:testId'))
            return false;
        if (resolvedBy.startsWith('semantic:role'))
            return false;
        if (resolvedBy.startsWith('semantic:label'))
            return false;
        // css-fallback: 대체 CSS 셀렉터 → 구조 기반이므로 비교적 안정
        if (resolvedBy.startsWith('css-fallback:'))
            return false;
 ...
 * fallback을 카테고리로 분류
 */
function categorizeFallback(resolvedBy, platform) {
    if (isCoordinateFallback(resolvedBy))
        return 'coordinate-fallback';
    if (isForceClick(resolvedBy))
        return 'force-click';
    if (platform === 'web') {
        if (resolvedBy.startsWith('css-fallback:'))
            return 'css-fallback';
        if (resolvedBy.startsWith('semantic:testId'))
            return 'semantic-testId';
        if (resolvedBy.startsWith('semantic:role'))
            return 'semantic-role';
        if (resolvedBy.startsWith('semantic:label'))
            return 'semantic-label';
        if (resolvedBy.startsWith('semantic:placeholder'))
            return 'semantic-placeholder';
        if (resolvedBy.startsWith('semantic:title'))
            return 'semantic-title';
        if (resolvedBy.startsWith('semantic:text'))
            return 'semantic-text';
        if (resolvedBy.startsWith('text-scoped:'))
            return 'text-scoped';
        return 'web-other';
    }
    if (platform === 'ios') {
        if (resolvedBy.includes('meta_element'))
            return 'ios-meta-element';
        return 'ios-other';
    }
    if (platform === 'android') {
        if (resolvedBy.includes('meta_element'))
            return 'android-meta-element';
