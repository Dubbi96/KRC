...
const chain_runner_1 = require("../dashboard/chain-runner");
/** 플랫폼에 맞는 리플레이어로 시나리오를 실행 */
async function replayByPlatform(scenario, options) {
...
async function handleMessage(msg) {
    const storage = new file_storage_1.FileStorage(msg.scenarioDir);
    const composer = new composer_1.ScenarioComposer(storage);
    try {
        switch (msg.type) {
            case 'run-single': {
                ...
                const composed = { ...scenario, events: composedEvents };
            ...
            case 'run-chain': {
                const chainRunner = new chain_runner_1.ChainRunner(storage);
