...
class AssertionEngine {
    ...
    _visualCheckCache = null;
    /** 단일 어설션 평가 */
    async evaluate(assertion, ctx) {
    ...
    /** 복수 어설션 평가 */
    async evaluateAll(assertions, ctx) {
    ...
    // ─── URL 검증 ───────────────────────────────────────────
    checkUrl(assertion, ctx, check) {
        if (!ctx.page)
            return { assertion, passed: false, error: 'No page context' };
    ...
    // ─── Element 검증 ──────────────────────────────────────
    async checkElement(assertion, ctx, selector, shouldExist) {
        if (!ctx.page)
            return { assertion, passed: false, error: 'No page context' };
    ...
    async checkElementVisible(assertion, ctx, selector) {
        if (!ctx.page)
            return { assertion, passed: false, error: 'No page context' };
        try {
    ...
    // ─── Text 검증 ──────────────────────────────────────────
    async checkPageText(assertion, ctx, check) {
        if (!ctx.page)
            return { assertion, passed: false, error: 'No page context' };
    ...
    async checkElementText(assertion, ctx, selector, check) {
        if (!ctx.page)
            return { assertion, passed: false, error: 'No page context' };
        try {
    ...
    async checkElementAttribute(assertion, ctx, selector, attribute, expected) {
        if (!ctx.page)
            return { assertion, passed: false, error: 'No page context' };
        try {
    ...
    // ─── HTTP 검증 ──────────────────────────────────────────
    checkHttpStatus(assertion, ctx, expected) {
        if (!ctx.lastApiResponse)
            return { assertion, passed: false, error: 'No API response available' };
    ...
    checkResponseBody(assertion, ctx, expected) {
        if (!ctx.lastApiResponse)
            return { assertion, passed: false, error: 'No API response available' };
    ...
    // ─── Variable 검증 ─────────────────────────────────────
    checkVariable(assertion, ctx, name, expected) {
    ...
    async checkCustom(assertion, ctx, expression) {
        if (!ctx.page)
            return { assertion, passed: false, error: 'No page context for custom assertion' };
        try {
  ...
  `;
     ...
     */
    async checkVideoPlaying(assertion, ctx, selector) {
        if (!ctx.page)
            return { assertion, passed: false, error: 'No page context' };
      ...
      return {
        ...
        try {
            // 1단계: 초기 상태 측정
            const t0State = await ctx.page.evaluate(videoStateScript, selector);
            if (!t0State) {
                // 폴백 1: iframe 내부에서 <video> 탐색 (Playwright frames API — cross-origin 지원)
                const iframeState = await this.findVideoStateInFrames(ctx.page, selector);
                if (iframeState) {
                    ...
                    const iframeT1 = await this.findVideoStateInFrames(ctx.page, selector);
                    if (iframeT1) {
            ...
            await new Promise(r => setTimeout(r, observeMs));
            // 3단계: 두 번째 상태 측정
            const t1State = await ctx.page.evaluate(videoStateScript, selector);
            if (!t1State) {
                return {
     ...
     */
    async checkVideoNoError(assertion, ctx, selector) {
        if (!ctx.page)
            return { assertion, passed: false, error: 'No page context' };
        try {
        ...
        return {
          ...
          readyState: el.readyState,
            ...
            if (!state) {
                ...
                const iframeState = await this.findVideoErrorStateInFrames(ctx.page);
                if (iframeState) {
     ...
     */
    async checkVideoAuto(assertion, ctx, selector) {
        if (!ctx.page)
            return { assertion, passed: false, error: 'No page context' };
     ...
     */
    async findVideoStateInFrames(page, selector) {
        const stateScript = `() => {
     ...
     */
    async findVideoErrorStateInFrames(page) {
        const errorScript = `() => {
      var el = document.querySelector('video');
      if (!el) return null;
      var err = el.error;
     ...
     */
    async checkVideoVisual(assertion, ctx, selector) {
        if (!ctx.page)
            return { assertion, passed: false, error: 'No page context' };
        ...
        try {
            ...
            if (buf1.length !== buf2.length) {
                ...
                if (this._visualCheckCache)
                    this._visualCheckCache.set(cacheKey, result);
                return result;
            }
            for (let i = 0; i < len; i++) {
                if (buf1[i] !== buf2[i])
     ...
     */
    checkStreamSegmentsLoaded(assertion, ctx) {
        ...
        const actual = `manifests=${manifestCount}, segments=${segmentCount}, ` +
    ...
    iosPageSourceCache = null;
    static IOS_CACHE_TTL = 2000; // 2초
    async getIOSPageSource(ctx) {
     ...
     */
    async checkIOSElementVisible(assertion, ctx) {
        if (!ctx.iosController)
            return { assertion, passed: false, error: 'No iOS controller context' };
        const xml = await this.getIOSPageSource(ctx);
        if (!xml)
            return { assertion, passed: false, error: 'Failed to get iOS pageSource' };
        ...
        if (!selector)
            return { assertion, passed: false, error: 'No iosSelector specified' };
     ...
     */
    async checkIOSElementNotExists(assertion, ctx) {
        if (!ctx.iosController)
            return { assertion, passed: false, error: 'No iOS controller context' };
        const xml = await this.getIOSPageSource(ctx);
        if (!xml)
     ...
     */
    checkIOSTextContains(assertion, ctx, expected) {
        // 동기 메서드 — 캐시된 pageSource 사용 (evaluateAll에서 순서 보장)
        if (!ctx.iosController)
            return { assertion, passed: false, error: 'No iOS controller context' };
        if (!this.iosPageSourceCache)
            return { assertion, passed: false, error: 'No cached iOS pageSource (call ios_element_visible first or use async variant)' };
     ...
     */
    async checkIOSElementValueEquals(assertion, ctx, expected) {
        if (!ctx.iosController)
            return { assertion, passed: false, error: 'No iOS controller context' };
        const xml = await this.getIOSPageSource(ctx);
        if (!xml)
     ...
     */
    async checkIOSTextAbsent(assertion, ctx, expected) {
        if (!ctx.iosController)
            return { assertion, passed: false, error: 'No iOS controller context' };
        const xml = await this.getIOSPageSource(ctx);
        if (!xml)
     ...
     */
    async checkIOSListCount(assertion, ctx) {
        if (!ctx.iosController)
            return { assertion, passed: false, error: 'No iOS controller context' };
        const xml = await this.getIOSPageSource(ctx);
        if (!xml)
     ...
     */
    async checkIOSNoAlert(assertion, ctx) {
        if (!ctx.iosController)
            return { assertion, passed: false, error: 'No iOS controller context' };
        const xml = await this.getIOSPageSource(ctx);
        if (!xml)
     ...
     */
    async checkIOSScreenChanged(assertion, ctx) {
        if (!ctx.iosController)
            return { assertion, passed: false, error: 'No iOS controller context' };
        const currentXml = await this.getIOSPageSource(ctx);
        if (!currentXml)
            return { assertion, passed: false, error: 'Failed to get current iOS pageSource' };
        ...
        const { parsePageSource, diffUITrees } = await Promise.resolve().then(() => __importStar(require('../ios/page-source-utils')));
    ...
    filterNetworkLogs(ctx, config) {
        ...
        return ctx.networkLogs.filter(log => {
            ...
            if (config.urlIsRegex) {
                if (!new RegExp(config.urlPattern, 'i').test(log.url))
                    return false;
            }
            else {
                if (!log.url.includes(config.urlPattern))
                    return false;
            }
            // method 필터
            if (config.method && log.method && log.method.toUpperCase() !== config.method.toUpperCase())
                return false;
            return true;
        });
    }
     ...
     */
    checkNetworkRequestSent(assertion, ctx) {
        const config = assertion.networkConfig;
        if (!config)
            return { assertion, passed: false, error: 'No networkConfig specified' };
        if (!ctx.networkLogs || ctx.networkLogs.length === 0) {
            return {
                assertion, passed: false,
     ...
     */
    checkNetworkResponseStatus(assertion, ctx) {
        const config = assertion.networkConfig;
        if (!config)
            return { assertion, passed: false, error: 'No networkConfig specified' };
        if (!ctx.networkLogs || ctx.networkLogs.length === 0) {
     ...
     */
    checkNetworkResponseJson(assertion, ctx) {
        const config = assertion.networkConfig;
        if (!config)
            return { assertion, passed: false, error: 'No networkConfig specified' };
        if (!config.jsonPath)
            return { assertion, passed: false, error: 'No jsonPath specified in networkConfig' };
        if (!ctx.networkLogs || ctx.networkLogs.length === 0) {
            return { assertion, passed: false, error: '네트워크 로그가 수집되지 않았습니다.' };
        }
     ...
     */
    checkNetworkImageLoads(assertion, ctx) {
        const config = assertion.networkConfig;
        if (!config)
            return { assertion, passed: false, error: 'No networkConfig specified' };
        if (!ctx.networkLogs || ctx.networkLogs.length === 0) {
        ...
        if (config.imageUrlJsonPath) {
            ...
            let body;
            try {
                body = JSON.parse(latestApi.responseBody);
            }
            catch {
            ...
            const results = sampled.map(url => {
                const imgLog = ctx.networkLogs.find(log => log.url.includes(url.substring(url.lastIndexOf('/') + 1)) && log.status === 200);
        ...
        const imageLogs = ctx.networkLogs.filter(log => log.timestamp >= cutoff && imagePatterns.test(log.url));
     ...
     */
    checkNetworkNoErrors(assertion, ctx) {
        const config = assertion.networkConfig;
        if (!config)
            return { assertion, passed: false, error: 'No networkConfig specified' };
        if (!ctx.networkLogs || ctx.networkLogs.length === 0) {
        ...
        const errorSummary = errors.slice(0, 5).map(e => `${e.status} ${e.method || 'GET'} ${e.url.substring(0, 60)}${e.error ? ` (${e.error})` : ''}`);
        return {
            assertion, passed,
            actual: passed
    ...
    resolveJsonPathForNetwork(obj, path) {
        if (!obj || !path)
            return undefined;
     ...
     */
    extractImageUrls(body, jsonPath) {
        // [*] 와일드카드 처리
