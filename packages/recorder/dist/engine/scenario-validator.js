...
exports.ScenarioValidator = void 0;
class ScenarioValidator {
    ...
    checkSelectorHealth(scenario) {
        const issues = [];
        scenario.events.forEach((ev, i) => {
            if (ev.disabled || !ev.selector)
                return;
            ...
            if (ev.meta?.selectors && ev.meta.selectors.length > 0) {
                const better = ev.meta.selectors.find((s) => s !== sel && (s.includes('data-testid') || s.startsWith('[role=') || s.includes('aria-label')));
                if (better) {
                    issues.push({
    ...
    checkVariableReferences(scenario) {
        const issues = [];
        // 정의된 변수 수집
        const defined = new Set(Object.keys(scenario.variables || {}));
        scenario.events.forEach((ev) => {
            if (ev.variableName)
                defined.add(ev.variableName);
            if (ev.extractData?.captureAs)
                defined.add(ev.extractData.captureAs);
            if (ev.ocrConfig?.targetVar)
                defined.add(ev.ocrConfig.targetVar);
            if (ev.apiRequest?.captureResponseAs)
                defined.add(ev.apiRequest.captureResponseAs);
            ...
            if (ev.forEachConfig?.itemVariable)
                defined.add(ev.forEachConfig.itemVariable);
            if (ev.forEachConfig?.countVariable)
                defined.add(ev.forEachConfig.countVariable);
            if (ev.script?.captureOutputAs)
                defined.add(ev.script.captureOutputAs);
        });
        // 내장 변수
        defined.add('__index');
        defined.add('__count');
    ...
    checkApiRequests(scenario) {
        const issues = [];
        scenario.events.forEach((ev, i) => {
            if (ev.disabled || ev.type !== 'api_request' || !ev.apiRequest)
                return;
    ...
    checkOnFailPolicies(scenario) {
        const issues = [];
        const totalSteps = scenario.events.length;
        scenario.events.forEach((ev, i) => {
            if (ev.disabled || !ev.onFail)
                return;
...
exports.ScenarioValidator = ScenarioValidator;
