...
const recorder_1 = require("./web/recorder");
const replayer_1 = require("./web/replayer");
const recorder_2 = require("./ios/recorder");
const replayer_2 = require("./ios/replayer");
const recorder_3 = require("./android/recorder");
...
async function loadOra() {
    return (await Promise.resolve().then(() => __importStar(require('ora')))).default;
}
...
function setupPauseToggle(recorder, chalk) {
...
async function recordWeb(opts, chalk, ora, inquirer) {
    const spinner = ora('Initializing web recording...').start();
    try {
        const storage = new file_storage_1.FileStorage(opts.output);
        // --continue: 기존 시나리오 이어서 녹화
        if (opts.continue) {
            spinner.text = 'Loading existing scenario...';
            const recorder = new recorder_1.WebRecorder({ outputDir: opts.output, browser: opts.browser, deviceType: opts.device || undefined, authProfileId: opts.auth || undefined }, storage);
        ...
        const recorder = new recorder_1.WebRecorder({ url, sessionName: name, outputDir: opts.output, browser: opts.browser, deviceType: opts.device || undefined, authProfileId: opts.auth || undefined }, storage);
...
async function recordIOS(opts, chalk, ora, inquirer) {
    ...
    try {
        ...
        if (!udid) {
            spinner.stop();
            try {
                const { listIOSDevices } = await Promise.resolve().then(() => __importStar(require('@katab/device-manager')));
                const devices = await listIOSDevices();
                if (devices.length === 0) {
                    console.error(chalk.red('No iOS devices found.'));
                    process.exit(1);
                }
                const answer = await inquirer.prompt([{
                        type: 'list', name: 'udid', message: 'Select device:',
                        choices: devices.map((d) => ({ name: `${d.name} (${d.version}) - ${d.udid}`, value: d.udid })),
                    }]);
                udid = answer.udid;
            }
            catch (e) {
        ...
        if (!bundleId) {
            spinner.stop();
            try {
                const { listIOSBundleIds } = await Promise.resolve().then(() => __importStar(require('@katab/device-manager')));
                const bundleIds = await listIOSBundleIds(udid);
                if (bundleIds.length > 0) {
                    const SKIP = '__skip__';
                    const INPUT = '__input__';
                    const choices = bundleIds.map((b) => ({ name: b, value: b }));
        ...
        const storage = new file_storage_1.FileStorage(opts.output);
        const recorder = new recorder_2.IOSRecorder(udid, {
        ...
        if (useMirror) {
            ...
            if (controller) {
                const { IOSMirrorServer } = await Promise.resolve().then(() => __importStar(require('./ios/mirror-server')));
...
async function recordAndroid(opts, chalk, ora, inquirer) {
    ...
    try {
        ...
        if (!deviceId) {
            spinner.stop();
            try {
                const { listAndroidDevices } = await Promise.resolve().then(() => __importStar(require('@katab/device-manager')));
                const devices = await listAndroidDevices();
                if (devices.length === 0) {
                    console.error(chalk.red('No Android devices found.'));
                    process.exit(1);
                }
                const answer = await inquirer.prompt([{
                        type: 'list', name: 'id', message: 'Select device:',
                        choices: devices.map((d) => ({ name: `${d.model} (${d.version}) - ${d.id}`, value: d.id })),
                    }]);
                deviceId = answer.id;
            }
            catch (e) {
        ...
        if (!pkg) {
            spinner.stop();
            try {
                const { listAndroidPackages, getCurrentAndroidPackage } = await Promise.resolve().then(() => __importStar(require('@katab/device-manager')));
        ...
        spinner.text = 'Connecting to device...';
        const storage = new file_storage_1.FileStorage(opts.output);
        const recorder = new recorder_3.AndroidRecorder(deviceId, { deviceId, package: pkg, appiumServerUrl: opts.appiumUrl, sessionName: name, outputDir: opts.output }, storage);
        const id = await recorder.start();
        // 미러 서버 시작
        let mirrorServer = null;
        if (useMirror) {
            const controller = recorder.getController();
            if (controller) {
                const { AndroidMirrorServer } = await Promise.resolve().then(() => __importStar(require('./android/mirror-server')));
...
program
    ...
    .action(async (scenarioFile, opts) => {
    const chalk = await loadChalk();
    const ora = await loadOra();
    ...
    try {
        ...
        if (scenarioFile.endsWith('.json')) {
            scenario = JSON.parse((0, fs_1.readFileSync)(scenarioFile, 'utf-8'));
        }
        else {
            scenario = await storage.loadScenario(scenarioFile);
            if (!scenario) {
                spinner.fail(chalk.red(`Not found: ${scenarioFile}`));
                process.exit(1);
            }
        }
        spinner.stop();
        ...
        if (scenario.platform === 'web') {
            result = await new replayer_1.WebReplayer().replay(scenario, replayOpts);
        }
        else if (scenario.platform === 'ios') {
            result = await new replayer_2.IOSReplayer().replay(scenario, replayOpts);
        }
        else if (scenario.platform === 'android') {
            result = await new replayer_3.AndroidReplayer().replay(scenario, replayOpts);
        }
        else {
            console.error(chalk.red(`Unsupported platform: ${scenario.platform}`));
            process.exit(1);
        }
        if (result) {
            ...
            console.log(chalk.cyan(`\n  Report: ${(0, path_1.join)(opts.report, scenario.id, 'report.html')}\n`));
...
program
    ...
    const chalk = await loadChalk();
    const inquirer = await loadInquirer();
    const { ScenarioEditor } = await Promise.resolve().then(() => __importStar(require('./editor/scenario-editor')));
    const storage = new file_storage_1.FileStorage(opts.output);
    ...
    while (running) {
        ...
        switch (action) {
                ...
                if (stepType === 'click' || stepType === 'fill') {
                    ...
                    if (stepType === 'fill') {
                        ...
                        if (value.includes('{{$') || value.includes('{{')) {
                            ...
                            if (capVar)
                                event.captureResolvedAs = capVar;
                        }
                    }
                    if (stepType === 'click') {
                    ...
                    if (aType === 'video_playing' || aType === 'video_no_error') {
                        ...
                        const assertion = { type: aType, expected: 'true', target: sel };
                    ...
                    const wfConfig = { waitType: wType, timeout: 10000 };
                    ...
                    const ifConfig = { conditionType: cType };
                    if (cType === 'element_exists' || cType === 'element_visible') {
                        ...
                        ifConfig.variable = vn;
                        ifConfig.expected = ve;
                    }
            ...
            case 'update': {
                ...
                if (!ev) {
                    console.log(chalk.red('Invalid step'));
                    break;
                }
                ...
                const { newVal } = await inquirer.prompt([{ type: 'input', name: 'newVal', message: `New ${field}:`, default: String(ev[field] || '') }]);
                const update = {};
                if (field === 'duration' || field === 'disabled')
                    update[field] = field === 'disabled' ? newVal === 'true' : parseInt(newVal);
                else
                    update[field] = newVal;
            ...
            case 'delete': {
                ...
                if (removed)
                    console.log(chalk.green(`Step #${idx} deleted`));
                else
                    console.log(chalk.red('Invalid step'));
                break;
            }
...
program
    ...
    const chalk = await loadChalk();
    const ora = await loadOra();
    const { TestRunner } = await Promise.resolve().then(() => __importStar(require('./engine/runner')));
    const storage = new file_storage_1.FileStorage(opts.output);
    ...
    try {
        ...
        for (const r of results) {
            ...
            const skipped = r.events.filter((e) => e.status === 'skipped').length;
            ...
            console.log(`       Report: ${(0, path_1.join)(opts.report, r.scenarioId, 'report.html')}`);
            console.log('');
            if (r.status === 'failed')
                allPassed = false;
        }
...
program
    ...
    const chalk = await loadChalk();
    const ora = await loadOra();
    const { ChainRunner } = await Promise.resolve().then(() => __importStar(require('./dashboard/chain-runner')));
    const storage = new file_storage_1.FileStorage(opts.output);
        ...
        const results = await chainRunner.runChain(ids, replayOpts, {
            ...
            onScenarioComplete: (_id, index, result) => {
                ...
                if (index < ids.length - 1)
                    spinner.start(`[${index + 2}/${ids.length}] 다음 시나리오 준비 중...`);
            },
        });
        spinner.stop();
        // 결과 요약
...
program
    ...
    try {
        ...
        const { IOSPickServer } = await Promise.resolve().then(() => __importStar(require('./ios/pick-server')));
...
program
    ...
    .action(async (opts) => {
    const chalk = await loadChalk();
    const { DashboardServer } = await Promise.resolve().then(() => __importStar(require('./dashboard/dashboard-server')));
...
program
    ...
    try {
        const { getDatabase } = await Promise.resolve().then(() => __importStar(require('./orchestrator/db/client')));
        const { getRedisConnectionFromUrl } = await Promise.resolve().then(() => __importStar(require('./orchestrator/queue/connection')));
        const { BaseWorker } = await Promise.resolve().then(() => __importStar(require('./orchestrator/worker/base-worker')));
        const { ResourcesRepository } = await Promise.resolve().then(() => __importStar(require('./orchestrator/db/repositories/resources')));
        ...
        if (!existing) {
            resourcesRepo.createResource(opts.resourceId, target, opts.resourceId);
...
resourceCmd
    ...
    try {
        ...
        repo.createResource(resourceId, target, name, metadata);
...
program
    ...
    try {
        const storage = new file_storage_1.FileStorage(opts.output);
        const { PartialReRecorder } = await Promise.resolve().then(() => __importStar(require('./web/partial-re-recorder')));
...
program
    ...
    const { existsSync, readdirSync } = await Promise.resolve().then(() => __importStar(require('fs')));
